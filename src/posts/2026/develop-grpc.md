...
Не в пример клиенту написать `gRPC` сервер, по крайней мере на [amphp/http-server](https://github.com/amphp/http-server), проще, потому что доступ к объекту запроса, тело которого `amphp` будет [собирать](https://github.com/amphp/http-server/blob/3.x/src/Driver/Http2Driver.php#L1099-L1112) самостоятельно, есть сразу, а объект ответа создаем мы:

```php
#[\Override]
public function handleRequest(Request $request): Response
{
    $response = new Response(status: HttpStatus::OK, headers: [
...
Для реализации серверных стримов любого типа так же используются две очереди[^5], проходящие через (де)сериализацию:
```php
#[\Override]
public function handleRequest(Request $request): Response
{
    ...
    $in = $this->codec->decode($request->getBody(), $rpc->type);

    return $response;
}
```

Поскольку трейлеры отправляются в самом конце, после того как закончится стрим, а ответ надо вернуть сейчас, `amphp` оставил возможность добавить трейлеры через фьючу:
...
Эта фьюча завершится, когда у стрима будет вызван метод `ConcurrentServerStream::close()`, и вернет все накопленные стримом трейлеры, которые могли добавить интерцепторы или декораторы.

Обработку запроса надо делать конкурентно, иначе не получится сделать серверный и двунаправленный стримы. Для этого можно использовать `EventLoop::queue()` или `async`. Возможно, второе даже лучше: так у нас появляется возможность реализовать `graceful shutdown`, дождавшись завершения всех фьюч.
...
public function handleRequest(Request $request): Response
{
    ...
    EventLoop::queue(
        $rpc->handle(...),
        new ConcurrentServerStream(
            $send,
            new Metadata($request->headers()),
            $trailers,
            $in,
        ),
    );
...
Работая со стримами, надо помнить о таймаутах, выставляемых `amphp` по умолчанию, которые годятся для обычных `http` запросов, призванных быть короткими, но не для стримов. Для запроса важно убрать [transferTimeout](https://github.com/amphp/http-client/blob/5.x/src/Request.php#L385) совсем, выставив 0, или указав достаточно большое число, если для общения с сервером будут использоваться не только унарные запросы. В конце концов, для этого придумали клиентский заголовок `grpc-timeout`. На стороне сервера надо [увеличить](https://github.com/amphp/http-server/blob/3.x/src/Driver/Http2Driver.php#L131) значение `streamTimeout`, который, как ни странно, выключить совсем, как `transferTimeout`, нельзя.

Хотя эти советы могут показаться вредными, поскольку все привыкли, что маленькие таймауты — это хорошо, надо помнить, что вы работаете со стримами, логику которых, в том числе хэртбиты, если это важно, можно описать в схеме взаимодействия сервиса. Взять, например, [ydb](https://github.com/ydb-platform/ydb-api-protos/blob/master/ydb_coordination_v1.proto#L22), обильно использующий стримы в общении с клиентами:
...
Клиент с сервером сами поймут, когда один из них перестал отвечать, и это куда лучше, чем вмешательство снаружи, где никто не осведомлен о состоянии стрима, чтобы его просто закрывать. В этот момент клиент может ждать подтверждения коммита транзакции, записи сообщения в очередь или завершения другой важной операции, потеря результата которой может привести к ошибкам.
...
Правда, ни возможность пользоваться такими системами полноценно, особенно двунаправленный стриминг, например топики в `ydb`, ни протокол общения между `php` приложениями, особенно со стороны сервера, со стандартным стеком от [Google](https://grpc.io/docs/languages/php/quickstart/) доступно не было. Отчасти виноваты умирающая модель работы и блокирующая природа языка, не позволяющие реализовать `gRPC` полноценно, отчасти — непопулярность и, хуже того, слабые компетенции в `php` внутри `Google`.

Впрочем, альтернативы уже были и до нас: это [Swoole](https://openswoole.com/docs/grpc), [Hyperf](https://github.com/hyperf/grpc-client) и [RoadRunner](https://docs.roadrunner.dev/docs/grpc/grpc). Однако они требуют установки дополнительных расширений и зависимостей, а также частично основаны на библиотеках от `Google` (например, все используют [google/protobuf](https://packagist.org/packages/google/protobuf)), что, на мой взгляд, усложняет интеграцию и распространение из-за необходимости разбираться, что из какой экосистемы берется и как превращается в один фреймворк. В то же время наш стек не использует существующие реализации, кроме, разумеется, `http/2` протокола, и поэтому его будет проще развивать и использовать. В следующей, заключительной, статье я расскажу, как принято писать кодогенератор для `gRPC` и `protobuf`.
...
[^5]: я намеренно опускаю детали реализации хранения `rpc`, поскольку это простая задача, не стоящая вашего внимания. Если в двух словах, это простая хэш-таблица с полным именем `rpc`, обработчиком (им могут быть `\Closure()` или `callable` типы) и входящим типом `protobuf` (или другого формата сериализации) сообщения. Выходящий тип (сообщения, отправляемые сервером) защищаются дженериком стрима. 

[^6]: разумеется, я утрирую. Скорее, это сложность внедрения необходимой инфраструктуры и понимания проблемы среди разработчиков в принципе.
